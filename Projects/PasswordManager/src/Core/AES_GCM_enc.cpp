...
int AES_GCM::encryptInit(const char *pw, size_t plen) {
	...
	memcpy(dst + crs, salt, SALT_SIZE);
	crs += SALT_SIZE;

	memcpy(dst + crs, iv, IV_SIZE);
	crs += IV_SIZE;
...
int AES_GCM::encryptBuff() {
	...
	if (EVP_EncryptUpdate(ctx, dst + crs, &res, src, size) != 1) {
		// LCOV_EXCL_START
